...
async function main() {
  ...
  const isPatientRegistered = await vitalChainCore.isRegisteredPatient(
    patient.address,
  );
  ...
  const hasDoctorRole = await vitalChainCore.hasRole(
    DOCTOR_ROLE,
    doctor.address,
  );
  if (!hasDoctorRole) {
  ...
  const hasEmergencyRole = await vitalChainCore.hasRole(
    EMERGENCY_ROLE,
    doctor.address,
  );
  if (!hasEmergencyRole) {
  ...
  const tx4 = await patientRecords
    .connect(doctor)
    .createRecord(
      patient.address,
      0,
  ...
  const tx5 = await patientRecords
    .connect(doctor)
    .createRecord(
      patient.address,
      0,
  ...
  const tx6 = await patientRecords
    .connect(doctor)
    .createRecord(
      patient.address,
      0,
  ...
  const tx7 = await patientRecords
    .connect(doctor)
    .createRecord(
      patient.address,
      0,
