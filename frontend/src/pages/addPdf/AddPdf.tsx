...
export default function AddPdf() {
  ...
  const uploadToIPFS = async () => {
    ...
    try {
      ...
      const message = `Upload PDF to IPFS\nTitle: ${formData.title}\nType: ${
        formData.recordType
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-600 mt-2">
          Add a new PDF medical record to IPFS and blockchain
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
            ...
            <div className="space-y-1 text-xs">
              ...
              <p
                className={
                  formData.recordType ? "text-green-600" : "text-red-600"
                }
              >
                {formData.recordType ? "✓" : "✗"} Record Type:{" "}
                {formData.recordType || "Not selected"}
              </p>
              ...
              <p
                className={
                  !servicesLoading ? "text-green-600" : "text-yellow-600"
                }
              >
                {!servicesLoading ? "✓" : "⏳"} Services loading:{" "}
                {servicesLoading ? "Yes" : "No"}
              </p>
              <p
                className={
                  services?.patientRecords ? "text-green-600" : "text-red-600"
                }
              >
                {services?.patientRecords ? "✓" : "✗"} PatientRecords service:{" "}
                {services?.patientRecords ? "Ready" : "Not ready"}
              </p>
            </div>
          </div>
          ...
          <p className="text-xs text-slate-500 text-center">
            Your PDF will be encrypted and stored on IPFS, with the hash
