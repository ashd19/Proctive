...
export class InsuranceClaimsService {
  ...
  // Approve claim
  async approveClaim(claimId: number, approvedAmount: string, notes: string): Promise<void> {
    try {
      const tx = await this.insuranceClaimsContract.approveClaim(claimId, approvedAmount, notes)
      await tx.wait()
    } catch (error: any) {
  ...
  async rejectClaim(claimId: number, reason: string): Promise<void> {
    try {
      const tx = await this.insuranceClaimsContract.rejectClaim(claimId, reason)
      await tx.wait()
    } catch (error: any) {
  ...
  async disputeClaim(claimId: number, reason: string): Promise<void> {
    try {
      const tx = await this.insuranceClaimsContract.disputeClaim(claimId, reason)
      await tx.wait()
    } catch (error: any) {
  ...
  async getPatientClaims(patientAddress: string): Promise<InsuranceClaim[]> {
    try {
      const claimIds = await this.insuranceClaimsContract.getPatientClaims(patientAddress)
  ...
  async getProviderClaims(providerAddress: string): Promise<InsuranceClaim[]> {
    try {
      const claimIds = await this.insuranceClaimsContract.getProviderClaims(providerAddress)
  ...
  async getClaimsByStatus(status: ClaimStatus): Promise<InsuranceClaim[]> {
    try {
      const claimIds = await this.insuranceClaimsContract.getClaimsByStatus(status)
